            ...
            if handling and handling.marking:
                for marking_spec in handling.marking: #Expects only 1 loop
                    ...
                    if isinstance(marking_spec.information_source, InformationSource):
                        ...
                        contributing_sources =  package.stix_header.information_source.contributing_sources
...
class AISInfoObject(AbstractMISPObjectGenerator):
    ...
    def get_template_id(self, misp, template_name="ais-info"):
        object_templates_list = misp.get_object_templates_list()['response']
        try:
            template_id = [x['ObjectTemplate']['id'] for x in object_templates_list if
                           x['ObjectTemplate']['name'] == template_name][0]
            return template_id
        except IndexError:
            valid_types = ", ".join([x['ObjectTemplate']['name'] for x in object_templates_list])
            logging.error("MISP Object Template for type %s not found on MISP Instance! Valid types are: %s" % (
                template_name, valid_types))
